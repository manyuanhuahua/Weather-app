...
const WeatherList = () => {
  ...
  // redirect user to daily page upon their selection
  const handleClick = (index) => {
    const daily = weather.list.filter(
      (item, i) => i >= index * 8 && i < (index + 1) * 8
    );
  ...
  return (
    <div className="weather-list-container">
      <h1 className="title">
        Next 5 days weather forcasting for{" "}
        <span style={{ color: "#ac3f21" }}>{weather.city.name}</span>
      </h1>
